...
convertVarscan2Annovar <- function () {
  ...
  fileNames<-list.files(arg, ".snp.pairs.tsv$")
...
#############################################eugenio: changed header to FALSE###################################################  
  ...
  fileNames<-list.files(arg, ".snp.pairs.tsv.somatic$")

  fileNamesIndels<-list.files(arg, ".indels.pairs.tsv$")
  ...
  for(i in 1:filesNum){
    ...
    CMDString<-paste0("java -jar ",  VARSCAN, "VarScan.v2.3.9.jar somaticFilter ", paste(getwd(), fileNames[i], sep="/"), 
                      " --indel-file ", paste(getwd(), fileNamesIndels[i], sep="/"), 
                      " --min-coverage 1 ", 
                      " --min-reads2 2 ", 
                      " --min-var-freq 0.1 ", 
  ...
  fileNames<-list.files(arg, ".snp.pairs.tsv.somatic.filtered$")
  ...
  for(i in 1:filesNum){
    ...
    tmpSet<-cbind(dset$chrom, dset$position, dset$position, dset[, 3:23], labels, stringsAsFactors = FALSE)
  ...
  write.table(annoSet, file="annoSetVarscan.tsv", row.names=F, col.names=F, sep="\t", quote=F)
  
  ### CALL ANNOVAR
  
  CMDString<-paste("perl ", annovarDIR, "table_annovar.pl ", arg, "/annoSetVarscan.tsv ",  annovarDIR, "humandb/ -buildver hg19 -out ", arg, "/somatic_mutation_varscan ", 
